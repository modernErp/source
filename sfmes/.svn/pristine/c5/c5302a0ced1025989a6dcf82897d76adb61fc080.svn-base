...
function make_csv_row(sheet, r, R, cols, fs, rs, FS, o) {
	...
	for(var C = r.s.c; C <= r.e.c; ++C) {
		...
		if(val == null) txt = "";
            ...
            if(val !== undefined && val.f != undefined &&(
            val.w == '#DIV/0!' || val.w == '#N/A' ||val.w =='#NAME?' ||val.w =='#NULL!'||  val.w =='#REF!'|| val.w == '#VALUE!'|| val.w=='#NUM!')) txt='';
            else if(val !== undefined &&val.t=='n' &&val.v != Number(val.w))
            {
				var df=val.w;
				if(typeof val.w == 'string' && val.v == Number(df.replaceAll(',','')))
				{
					txt=df.replaceAll(',','');
				}
				else
				{
					df=new Date(Math.round((val.v - 25569)*86400*1000)).toISOString();
					df=df.replaceAll('/','');
					df=df.replaceAll('-','');
					df=df.replaceAll('T','');
					df=df.replaceAll('Z','');                
