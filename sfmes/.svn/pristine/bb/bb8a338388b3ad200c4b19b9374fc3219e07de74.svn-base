...
*/
@Service("infCoSraNoHistService")
public class infCoSraNoHistServiceImpl extends CmnAbstractServiceImpl implements infCoSraNoHistService{
    ...
    // 축산물이력(묶음)번호 내역 조회
    @Override
    public LinkedHashMap<String, Object> infCoSraNoHistselect(LinkedHashMap paramMap01) throws Exception {
        // 축산물이력번호관리기본 출고항목 수정
        ...
        try {
            ...
            // 공공데이타 자료 확인
            rst_ParamMap = infConnectDataHistNo(paramMap01);
            ...
            if ("".equals(rst_ParamMap.get("data")) || rst_ParamMap.get("data") != null) {
                if ('L' == sub_HstNo[0]) {
                    ...
                    // 묶음번호 등록
                    resultMap = infCoDataBudlNoHistInsert(paramMap01, rst_ParamMap);                    
                } else {

                    // 개별축산물이력번호 등록
                    resultMap = infCoDataIdntNoHistInsert(paramMap01, rst_ParamMap);
                }
            }
...
//        return sqlSession.selectList("sfmes.sqlmap.co.selectCo2030_List01",paramMap01);
    ...
    @Override
    public void infCoSraNoHistupdate(LinkedHashMap paramMap01) throws Exception  {
        //축산물이력(묶음)번호기본 내역 수정
    ...
    @Override
    public LinkedHashMap<String, Object> infConnectDataHistNo(LinkedHashMap paramMap) throws Exception {
        // 축산물이력번호관리기본 출고항목 수정
    ...
    @Override
    public LinkedHashMap<String, Object> infCoDataBudlNoHistInsert(LinkedHashMap paramMap, LinkedHashMap paramMap01) throws Exception {
        LinkedHashMap<String, Object> resultMap = new LinkedHashMap<>();
        ...
        try 
        {
            ...
            for (Map<String, Object> budl_map : itemList) {
                ...
                if ("8".equals(infoType)) {
                    ...
                    resultMap_dist = infConnectDataHistNo((LinkedHashMap) budl_map);
                    ...
                    // 개별축산물이력번호 등록
                    resultMap = infCoDataIdntNoHistInsert(paramMap, resultMap_dist);
...
//            egovLogger.debug("######################################");
    ...
    @Override
    public LinkedHashMap<String, Object> infCoDataIdntNoHistInsert(LinkedHashMap paramMap, LinkedHashMap paramMap01) throws Exception {    
        // 축산물이력번호관리기본 출고항목 수정
        ...
        Map<String, Object> m_idnt_map = new HashMap<>(); // 개체정보
        Map<String, Object> d_sra_map = new HashMap<>();  // 출생신고
        Map<String, Object> d_buc_map = new HashMap<>();  // 도축신고
        Map<String, Object> d_4_map = new HashMap<>();  // 포장신고내역
        Map<String, Object> d_5_map = new HashMap<>();  // 구제역정보
        Map<String, Object> d_6_map = new HashMap<>();  // 질병정보
        Map<String, Object> d_7_map = new HashMap<>();  // 브루셀라여부        
...
//                    egovLogger.debug("### infoType  =>"+infoType);
                    ...
                    if ("1".equals(infoType)) {
                        ...
                        if ("CATTLE".equals(t_Type)) {
                            ...
                            // paramMap 을 변경하는 이유 ===> 410 이 빠지는 경우 존재함.
                            t_map.put("SRA_HST_NO", t_map.get("cattleNo"));
                        } else {
                        ...
                        // 출생_생축 신고내역 ===> 마지막자료만 저장
                        m_idnt_map.put("FAM_ADR"    , t_map.get("farmAddr"));   // 사육지주소
                        m_idnt_map.put("FAM_NM"     , t_map.get("farmerNm"));   // 소유주
                        m_idnt_map.put("FAM_REG_TP" , t_map.get("regType"));    // 신고구분
                        m_idnt_map.put("REG_DT"     , t_map.get("regYmd"));     // 년월일
                        m_idnt_map.put("FAM_NO"     , t_map.get("farmNo"));     // 농장번호
                        
                    } else if ("4".equals(infoType)) {
                        // 포장내역
                        m_idnt_map.put("PCSPL_ADR"  , t_map.get("processPlaceAddr"));   // 포장처리업소 주소
                        m_idnt_map.put("PCSPL_NM"   , t_map.get("processPlaceNm"));     // 포장처리업소명

                    } else if ("5".equals(infoType)) {
                        // 구제역정보
                        m_idnt_map.put("INJECT_DAYCNT" , t_map.get("injectionDayCnt"));   // 구제역 백신접종경과일
                        m_idnt_map.put("INJECT_DT"     , t_map.get("injectionYmd"));   // 구제역 백신접종일
                        m_idnt_map.put("INJECT_ODR"    , t_map.get("vaccineorder"));   // 구제역 백신접종 차수

                    } else if ("6".equals(infoType)) {
                        // 질병정보
                        m_idnt_map.put("INSP_DESC" , t_map.get("inspectDesc"));   // 질병유무

                    } else if ("7".equals(infoType)) {
                        // 브루셀라여부
                        m_idnt_map.put("INSPECT_DT" , t_map.get("gradeNm"));   // 브루셀라 접종일
                        m_idnt_map.put("INSPECT_YN" , t_map.get("insfat"));    // 브루셀라 접종유무

                    } else if ("3".equals(infoType)) {
                        // 등급명
                        if ("CATTLE".equals(t_Type)) {
                            m_idnt_map.put("GRD_NM" , t_map.get("gradeNm"));
                            m_idnt_map.put("INSFAT" , t_map.get("insfat"));       /* 20201204 JKS 추가 근내지방도         */
                            ...
                            m_idnt_map.put("BUTCHER_NM"  , t_map.get("butcheryPlaceNm"));   // 도축장명
                            m_idnt_map.put("INSP_PASSYN" , t_map.get("inspectPassYn"));     // 합격여부
                            m_idnt_map.put("BUTCHER_ADR" , t_map.get("butcheryPlaceAddr")); // 도축장주소
                            m_idnt_map.put("BUTCHER_DT"  , t_map.get("butcheryYmd"));       // 도축일자
                        } else if ("PIG".equals(t_Type)) {
                            ...
                            m_idnt_map.put("INSFAT"    , "");                     /* 20201204 JKS 추가 근내지방도         */
                        }                        
...
//                    egovLogger.debug("######################################");
    ...
    @Override
    public LinkedHashMap<String, Object> infCoMeatwatchHistselect(LinkedHashMap paramMap) throws Exception {
        // 축산물이력번호관리기본 출고항목 수정
        ...
        try 
        {
            ...
            if(len_hst_no == 24) {
                ...
                resultMap = infCoMeatwatchIdntNoHistselect(paramMap, resultMap_01);
            }
    ...
    @Override
    public LinkedHashMap<String, Object> infConnectCoMeatWatch(LinkedHashMap paramMap) throws Exception {
        // 축산물이력번호관리기본 출고항목 수정
        egovLogger.debug("************ 국내산 - 축산물이력(묶음)번호 내역 조회 (공공데이타접속)[connectDataHistNo] *********");
        egovLogger.debug("paramMap: "+paramMap.toString());
    ...
    @Override
    public LinkedHashMap<String, Object> infCoMeatwatchBudlNoHistselect(LinkedHashMap paramMap, LinkedHashMap paramMap01)  throws Exception {
        LinkedHashMap<String, Object> resultMap      = new LinkedHashMap<>();
        LinkedHashMap<String, Object> resultMap_dist = new LinkedHashMap<>();
        ...
        try 
        {
            ...
            for (Map<String, Object> t_map_1 : paramList) {
                ...
                resultMap_dist = infConnectCoMeatWatch(paramMap);
                ...
                // 유통삭뵬번호 조회 - 이력관리기본 insert
                d_budl_map = infCoMeatwatchIdntNoHistselect(paramMap, resultMap_dist);
                ...
                d_budl_map.put("MFC_DSC"             , "1");                              // 가공구분은 1 : 자체생산으로 입력
...
//            egovLogger.debug("### resultMap="+resultMap);
    ...
    // 수입축산물묶음번호 내역 조회 On-line
    @Override
    public LinkedHashMap<String, Object> infCoMeatwatchIdntNoHistselect(LinkedHashMap paramMap, LinkedHashMap paramMap01) throws Exception {
        LinkedHashMap<String, Object> resultMap = new LinkedHashMap<>();
